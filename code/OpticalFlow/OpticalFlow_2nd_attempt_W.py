...
"""
In this revised code, the x, y, u, and v variables are explicitly cast to integers
using the int function before being passed to the cv2.line and cv2.circle functions.
This should ensure that the arguments are in the expected format and prevent the 
"Bad argument" error from occurring.
"""
...
while True:
    ...
    for i, (prev_pt, next_pt) in enumerate(zip(good_prev_pts, good_next_pts)):
        x, y = prev_pt.ravel()
        u, v = next_pt.ravel()
        mask = cv2.line(mask, (int(x), int(y)), (int(u), int(v)), (0, 255, 0), 2)
        frame = cv2.circle(frame, (int(u), int(v)), 5, (0, 255, 0), -1)
